...
export async function POST(request: NextRequest) {
    try {
        ...
        const suggestionsUrl = process.env.CREWAI_SUGGESTIONS_URL;
        if (!suggestionsUrl) {
            return NextResponse.json(
                { error: "CREWAI_SUGGESTIONS_URL not configured" },
                { status: 500 }
            );
        }

        // Submit analysis to FastAPI backend
        const submitResponse = await fetch(`${suggestionsUrl}/api/analyze`, {
            method: "POST",
            headers: {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            const statusResponse = await fetch(
                `${suggestionsUrl}/api/results/${analysisId}`
            );
