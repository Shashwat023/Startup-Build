...
class AgenticRAGPipeline:
    ...
    def __init__(self, chunk_size=500, chunk_overlap=50, session_id=None):
        ...
        print(
            f"[SUCCESS] Enhanced Agentic RAG initialized for session: {self.session_id}"
        )
    ...
    def process_image_multimodal(
        self, image_path: str, source_file: str
    ) -> Dict[str, str]:
...
Decision:""",
            ...
            content_type = (
                content_chain.invoke({"question": state["question"]}).strip().upper()
            )
...
Answer:""",
            ...
            chain = prompt | self.generator_llm | StrOutputParser()
            answer = chain.invoke(
                {"history": state["chat_history"], "question": state["question"]}
            )
...
if __name__ == "__main__":
    ...
    print(
        f"Text docs: {result1['text_docs_count']}, Image docs: {result1['image_docs_count']}"
    )
    ...
    print(
        f"Text docs: {result2['text_docs_count']}, Image docs: {result2['image_docs_count']}"
    )
    ...
    print(
        f"Text docs: {result3['text_docs_count']}, Image docs: {result3['image_docs_count']}"
    )
