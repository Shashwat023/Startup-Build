...
from main import prepare_inputs  # noqa: E402
from crew import BoardPanelCrew  # noqa: E402
...
AGENT_PIPELINE = [
    {
        ...
        "display": "Finance Strengths",
        "result_key": "finance_strengths",
    },
    {
        "agent": "marketing_advisor",
        "task": "marketing_analysis_task",
        "display": "Marketing Strengths",
        "result_key": "marketing_strengths",
    },
    {
        "agent": "tech_lead",
        "task": "tech_analysis_task",
        "display": "Tech Strengths",
        "result_key": "tech_strengths",
    },
    {
        "agent": "org_hr_strategist",
        "task": "org_hr_analysis_task",
        "display": "Org/HR Strengths",
        "result_key": "org_hr_strengths",
    },
    {
        "agent": "competitive_analyst",
        "task": "competitive_analysis_task",
        "display": "Competitive Strengths",
        "result_key": "competitive_strengths",
    },
]
    ...
    try:
        ...
        if hasattr(task_output, "pydantic"):
            ...
            if isinstance(pydantic_output, AgentStrengthOutput):
                ...
                print(
                    f"✓ Task {task_index} ({task_name}): Dict from pydantic attribute"
                )
                return pydantic_output["strengths"]
        ...
        if isinstance(task_output, dict):
            if "strengths" in task_output:
                ...
                return task_output["strengths"]
            elif "pydantic" in task_output and isinstance(
                task_output["pydantic"], dict
            ):
        ...
        strengths_match = re.search(
            r'"strengths"\s*:\s*\[(.*?)\]', output_str, re.DOTALL
        )
        if strengths_match:
...
async def run_single_agent_with_retry(
    crew: BoardPanelCrew, agent_info: dict, inputs: dict, agent_status: AgentStatus
) -> Optional[str]:
    """
        ...
        try:
            ...
            result = await asyncio.get_event_loop().run_in_executor(
                ...
                lambda: crew.run_single_task(
                    agent_info["agent"], agent_info["task"], inputs
                ),
            )
            ...
            task_index = next(
                (
                    i
                    for i, info in enumerate(AGENT_PIPELINE)
                    if info["agent"] == agent_info["agent"]
                ),
                0,
            )
            ...
            if attempt <= MAX_RETRIES_PER_AGENT:
                ...
                agent_status.error = (
                    f"Attempt {attempt} failed, will retry after cooldown"
                )
                print(f"[PIPELINE] Will retry after {COOLDOWN_SECONDS}s cooldown...")
            else:
                # Final failure - use fallback strengths
                agent_status.status = (
                    "completed"  # Mark as completed to continue pipeline
                )
                agent_status.completed_at = datetime.now().isoformat()
                task_index = next(
                    (
                ...
                agent_status.error = (
                    f"Failed after {attempt} attempts, using fallback strengths"
                )
...
async def run_sequential_pipeline(analysis_id: str, startup_data: StartupInput):
    ...
    try:
        ...
        for i, agent_info in enumerate(AGENT_PIPELINE):
            ...
            print(f"\n{'#' * 60}")
            print(
                f"[PIPELINE] Starting Agent {i + 1}/{len(AGENT_PIPELINE)}: {agent_info['display']}"
            )
            print(f"{'#' * 60}")
            ...
            if i < len(AGENT_PIPELINE) - 1:
                ...
                print("[PIPELINE] ✓ Cooldown complete, proceeding to next agent\n")
        ...
        print(f"\n{'=' * 60}")
        print(
            f"[PIPELINE] Complete! {len(completed_agents)}/{len(AGENT_PIPELINE)} agents completed"
        )
        print(f"{'=' * 60}")
...
async def analyze(request: AnalysisRequest, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(
        run_sequential_pipeline, analysis_id, request.startup_data
    )
