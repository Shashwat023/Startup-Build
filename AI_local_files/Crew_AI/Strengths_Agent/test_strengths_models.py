...
class TestAgentStrengthOutput:
    def test_accepts_three_to_five_strengths(self):
        output = AgentStrengthOutput(
            agent_name="Marketing",
            strengths=["Strength one.", "Strength two.", "Strength three."],
        )
        assert len(output.strengths) == 3

    def test_rejects_fewer_than_three_strengths(self):
        with pytest.raises(ValidationError):
            AgentStrengthOutput(
                agent_name="Marketing",
                strengths=["Only one.", "Only two."],
            )

    def test_rejects_more_than_five_strengths(self):
        with pytest.raises(ValidationError):
            AgentStrengthOutput(
                agent_name="Marketing",
    ...
    def test_requires_agent_name(self):
        with pytest.raises(ValidationError):
            AgentStrengthOutput(strengths=["A.", "B.", "C."])
...
class TestStartupInput:
    ...
    def test_rejects_invalid_funding_status(self):
        with pytest.raises(ValidationError):
            make_startup_input(finance_runway=FinanceRunway(funding_status="Series Z"))


class TestStrengthsResults:
    def test_defaults_are_independent_empty_lists(self):
        a = StrengthsResults()
        b = StrengthsResults()
        a.marketing_strengths.append("x")
        assert b.marketing_strengths == []
...
class TestAgentStatus:
    def test_rejects_invalid_status_literal(self):
        with pytest.raises(ValidationError):
            AgentStatus(
                agent_name="tech_lead", display_name="Tech Lead", status="unknown"
            )
...
class TestPrepareInputs:
    ...
    def test_empty_lists_fall_back_to_placeholder_text(self):
        startup = make_startup_input(
            competition_market=CompetitionMarket(known_competitors=[])
        )
        inputs = prepare_inputs(startup)
        assert inputs["competitors"] == "None specified"
