...
AGENT_PIPELINE = [
    {
        ...
        "display": "Finance Suggestions",
        "result_key": "finance_suggestions",
    },
    {
        "agent": "marketing_advisor",
        "task": "marketing_analysis_task",
        "display": "Marketing Suggestions",
        "result_key": "marketing_suggestions",
    },
    {
        "agent": "tech_lead",
        "task": "tech_analysis_task",
        "display": "Tech Suggestions",
        "result_key": "tech_suggestions",
    },
    {
        "agent": "org_hr_strategist",
        "task": "org_hr_analysis_task",
        "display": "Org/HR Suggestions",
        "result_key": "org_hr_suggestions",
    },
    {
        "agent": "competitive_analyst",
        "task": "competitive_analysis_task",
        "display": "Competitive Suggestions",
        "result_key": "competitive_suggestions",
    },
]
...
def extract_suggestions_from_output(task_output, agent_name: str) -> list:
    ...
    try:
        ...
        suggestions_match = re.search(
            r'"suggestions"\s*:\s*\[(.*?)\]', output_str, re.DOTALL
        )
...
async def run_single_agent_with_retry(
    ...
    for attempt in range(1, MAX_RETRIES_PER_AGENT + 2):  # 1 initial + 1 retry
        ...
        try:
            ...
            suggestions = extract_suggestions_from_output(
                task_output, agent_info["agent"]
            )
            ...
            if attempt <= MAX_RETRIES_PER_AGENT:
                ...
                agent_status.error = (
                    f"Failed after {attempt} attempts, using fallback suggestions"
                )
                print(
                    f"[PIPELINE] ⚠ {agent_info['display']} using fallback suggestions"
                )
                return None
