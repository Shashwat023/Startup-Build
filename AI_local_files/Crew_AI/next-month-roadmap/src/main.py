...
def prepare_inputs(startup_data: StartupInput) -> dict:
    ...
    # Marketing inputs
    marketing_channels = (
        ", ".join(startup_data.marketing_growth.current_marketing_channels)
        if startup_data.marketing_growth.current_marketing_channels
        else "None specified"
    )

    # Tech inputs
    current_features = (
        ", ".join(startup_data.product_technology.current_features)
        if startup_data.product_technology.current_features
        else "None specified"
    )
    tech_stack = (
        ", ".join(startup_data.product_technology.tech_stack)
        if startup_data.product_technology.tech_stack
        else "None specified"
    )

    # Org inputs
    founder_roles = (
        ", ".join(startup_data.team_organization.founder_roles)
        if startup_data.team_organization.founder_roles
        else "None specified"
    )

    # Competition inputs
    competitors = (
        ", ".join(startup_data.competition_market.known_competitors)
        if startup_data.competition_market.known_competitors
        else "None specified"
    )
    ...
    return {
        ...
        "retention_strategy": startup_data.marketing_growth.retention_strategy
        or "No formal strategy",
        "growth_problems": startup_data.marketing_growth.growth_problems
        or "None specified",
        # Tech inputs
        ...
        "tech_challenges": startup_data.product_technology.tech_challenges
        or "None specified",
        # Org inputs
        "team_size": str(startup_data.team_organization.team_size),
        "founder_roles": founder_roles,
        "hiring_plan": startup_data.team_organization.hiring_plan_next_3_months
        or "No formal plan",
        "org_challenges": startup_data.team_organization.org_challenges
        or "None specified",
        # Competition inputs
        "competitors": competitors,
        "unique_advantage": startup_data.competition_market.unique_advantage
        or "Not clearly defined",
        "pricing_model": startup_data.competition_market.pricing_model or "Not defined",
        "market_risks": startup_data.competition_market.market_risks
        or "None identified",
        # Finance inputs
        ...
        "financial_concerns": startup_data.finance_runway.financial_concerns
        or "None specified",
    }


def run(startup_data: StartupInput):
    """Run the crew with startup data and retry logic for rate limits."""
        ...
        try:
            ...
            if "rate_limit" in error_msg.lower() or "RateLimitError" in error_msg:
                if attempt < max_retries - 1:
                    ...
                    print(
                        "2. Use a faster model with higher limits (e.g., llama-3.1-8b-instant)"
                    )
                    print(
                        "3. Consider upgrading your Groq plan at https://console.groq.com/settings/billing"
                    )
                    raise Exception(
