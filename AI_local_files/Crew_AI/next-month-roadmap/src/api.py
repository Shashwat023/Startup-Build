...
AGENT_PIPELINE = [
    {
        ...
        "display": "Finance Roadmap",
        "result_key": "finance_roadmap",
    },
    {
        "agent": "marketing_advisor",
        "task": "marketing_analysis_task",
        "display": "Marketing Roadmap",
        "result_key": "marketing_roadmap",
    },
    {
        "agent": "tech_lead",
        "task": "tech_analysis_task",
        "display": "Tech Roadmap",
        "result_key": "tech_roadmap",
    },
    {
        "agent": "org_hr_strategist",
        "task": "org_hr_analysis_task",
        "display": "Org/HR Roadmap",
        "result_key": "org_hr_roadmap",
    },
    {
        "agent": "competitive_analyst",
        "task": "competitive_analysis_task",
        "display": "Competitive Roadmap",
        "result_key": "competitive_roadmap",
    },
]
    ...
    for attempt in range(1, MAX_RETRIES_PER_AGENT + 2):  # 1 initial + 1 retry
        ...
        try:
            ...
            if attempt <= MAX_RETRIES_PER_AGENT:
                ...
                agent_status.completed_at = datetime.now().isoformat()
                print(
                    f"[PIPELINE] ✗ {agent_info['display']} failed after {attempt} attempts"
                )
                return None
...
async def run_sequential_pipeline(analysis_id: str, startup_data: StartupInput):
    ...
    try:
        ...
        for i, agent_info in enumerate(AGENT_PIPELINE):
            ...
            output = await run_single_agent_with_retry(
                crew, agent_info, inputs, agent_status
            )
        ...
        if len(completed_agents) > 0:
            ...
            if failed_agents:
                analysis_results[
                    ...
                    f"{len(failed_agents)} agent(s) failed, partial results available"
                )
        else:
        ...
        print(f"\n{'=' * 60}")
        print(
            f"[PIPELINE] Complete! {len(completed_agents)}/{len(AGENT_PIPELINE)} agents succeeded"
        )
        print(f"{'=' * 60}")
