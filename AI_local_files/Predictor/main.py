...
# ------------------------------
# Resolve relative to this file, not the process's cwd, so it works
# regardless of where the app/tests are launched from.
_PIPELINE_PATH = Path(__file__).parent / "xgb_pipeline.pkl"
pipeline = joblib.load(_PIPELINE_PATH)  # Make sure this file exists
...
def predict_startup(data: StartupData):
    ...
    return {"prediction": pred_label, "probability_acquired": round(pred_proba, 4)}
...
def predict_batch(data: List[StartupData]):
    ...
    for label, prob in zip(preds_labels, probs_list):
        results.append({"prediction": label, "probability_acquired": prob})
